...
exports.Solver = function(people) {
    ...
    this._blindPick = function () {
        var available = _.clone(people),
            assignments = [],
            personIndex;

        for(personIndex = 0; personIndex < people.length; personIndex++) {
            assignments.push({
                current: people[personIndex],
                chosen: available.splice(_.random(0, available.length() - 1), 1)
            });
        }

        return _.map(assignments, function(s) {
            return {
                from: s.current,
                to: s.chosen
            }
